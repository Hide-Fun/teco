...
parse_gbxml <- function(.xml, .gbseq = c("all", "non_seq", "manual")) {
  ...
  gbseq_list <- furrr::future_map(1:size, SplitGBset, .list = list, .options = furrr_options(seed = 1L), .progress = TRUE)
  feature_table_list <- furrr::future_map(gbseq_list, purrr::pluck, "GBSeq_feature-table", "GBFeature", "GBFeature_quals", .list = list, .options = furrr_options(seed = 1L), .progress = TRUE)
  other_seqids_list <- furrr::future_map(gbseq_list, purrr::pluck, "GBSeq_other-seqids", .options = furrr_options(seed = 1L), .progress = TRUE)
  references_list <- furrr::future_map(gbseq_list, purrr::pluck, "GBSeq_references", "GBReference", .options = furrr_options(seed = 1L), .progress = TRUE)
  ...
  gbseq <- furrr::future_map_dfr(gbseq_list, MapGetValue, .names = lev_gbeq, .options = furrr_options(seed = 1L), .progress = TRUE) %>%
  ...
  references <- furrr::future_map_dfr(references_list, MapGetValue, .names = lev_ref, .options = furrr_options(seed = 1L), .progress = TRUE) %>%
    dplyr::rename_with(stringr::str_replace_all,
                       .cols = dplyr::everything(),
                       pattern = repl_ref)
  ...
  feature_table <- feature_table_list %>%
    ...
    dplyr::mutate(name2 = furrr::future_map(value, names, .options = furrr_options(seed = 1L), .progress = TRUE),
                  value2 = furrr::future_map(value, unlist), .options = furrr_options(seed = 1L), .progress = TRUE) %>%
  ...
  other_seqids <- other_seqids_list %>%
    tibble::enframe(name = "id") %>%
    dplyr::mutate(value2 = furrr::future_map(value, unlist, .options = furrr_options(seed = 1L), .progress = TRUE),
                  value2 = furrr::future_map(value2, paste0, collapse = "_", .options = furrr_options(seed = 1L), .progress = TRUE)) %>%
...
MapGetValue <- function( .list, .names, .f = GetValue) {
  furrr::future_map_dfc(.names, GetValue, .list = .list, .options = furrr_options(seed = 1L), .progress = TRUE)
}
